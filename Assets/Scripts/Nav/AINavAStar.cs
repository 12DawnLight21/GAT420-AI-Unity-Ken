...
public class AINavAStar
{
		...
		while (!found && nodes.Count > 0) 
		{
            ...
            foreach (var neighbor in node.neighbors)
            {
				...
				if (cost < neighbor.Cost) 
				{
					...
					heuristic = Vector3.Distance(neighbor.transform.position, neighbor.transform.position);
					nodes.EnqueueWithoutDuplicates(neighbor, neighbor.Cost + heuristic); //cost might be halved by heuristic here, maple isnt sure
				}
            }
        }
	...
	public static void CreatePathFromParents(AINavNode node, ref List<AINavNode> path) // sets nodes to parents node and reverses order to make a path from A to F
	{
		// while node not null
		while (node != null)
		{
