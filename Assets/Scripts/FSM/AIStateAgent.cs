...
public class AIStateAgent : AIAgent
{
    ...
    private void Start()
    {
        ...
        //stateMachine.AddState(nameof(AIPatrolState), new AIPatrolState(this)); // mke htis hit
        stateMachine.AddState(nameof(AIDeathState), new AIDeathState(this));

        stateMachine.SetState(nameof(AIIdleState)); // nameof returns class name as a string
    }
